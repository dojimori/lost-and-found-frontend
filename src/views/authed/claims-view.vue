    ...
    <table class="table table-md">
      ...
      <tbody>
        <tr v-for="(claimed, index) in myClaimedItems" :key="index">
          ...
          <td>
             <img :src="`http://localhost:3000/public/${claimed.item?.image}`" class="w-[50px] h-[50px] rounded-md object-cover" :alt="claimed.item.name" />
          </td>
          <td>June 26, 2025</td>
          <td class="flex gap-2">
            <button class="btn btn-primary">Message Founder</button>
            <button class="btn btn-error btn-soft">Unclaim</button>
          </td>
        </tr>
      </tbody>
    </table>
